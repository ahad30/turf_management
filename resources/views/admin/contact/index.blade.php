    ...
    <div class="flex flex-col rounded-lg overflow-hidden">
        <div class=" bg-gray-100">
            <div class="inline-block min-w-full align-middle">
                                ...
                                <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                    ...
                                    <td class="p-4  space-x-2 whitespace-nowrap">
                                        <div class="flex gap-4">
                                            @if ($item->status == 0)
                                                <a href="{{ route('admin.showContactMessage', $item->id) }}">
                                                    <button type="button"
                                                        ...
                                                        <ion-icon name="eye-outline" class="text-xl mr-3"></ion-icon>
                                                        view
                                                    </button>
                                                </a>
                                                <a href="{{ route('admin.status', $item->id) }}">
                                                    <button type="button"
                                                        class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-gray-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
                                            ...
                                            <form action="{{ route('admin.deleteContactMessage', $item->id) }}"
                                                method="post">
                                                @method('DELETE')
                                                @csrf
                                                <button type="submit"
                ...
                <div class="w-full p-4 bg-white"> {{ $contacts->links() }}</div>
            </div>
        </div>
