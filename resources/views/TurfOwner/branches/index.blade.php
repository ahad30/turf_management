<x-dashboard-layout>
    ...
    <div class="w-full mt-4 mb-8 text-right">
        <a href="{{ route('turf-owner.branches.create') }}"
    ...
    <div class="flex flex-col rounded-lg overflow-hidden">
        <div class=" bg-gray-100">
            <div class="inline-block min-w-full align-middle">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                            @foreach ($branches as $branch)
                                <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                    ...
                                    <td class="p-4 ">
                                        {{ $branch->products->count() }}
                                    </td>
                                    <td class="p-4  space-x-2 whitespace-nowrap">
                                        <div class="flex gap-4">
                                            <a
                                                href="{{ route('turf-owner.branches.edit', ['branch' => $branch->id]) }}">
                                                <button type="button"
                                            ...
                                            <form
                                                action="{{ route('turf-owner.branches.destroy', ['branch' => $branch->id]) }}"
                                                method="post">
                                                @method('DELETE')
                                                @csrf
                                                <button type="submit"
                                                    onclick="return confirm('Are you sure you want to destroy this branch')"
                ...
                <div class="w-full p-4 bg-white"> {{ $branches->links() }}</div>
            </div>
        </div>
