    ...
    <div class="w-full mt-4 mb-3 text-left">
        <div class="flex justify-left">
            <a href="{{ route('turf-owner.products.index') }}"
                class="px-3 py-1 m-2 border rounded-sm bg-primary hover:bg-green-700 text-white">All Products</a>
            @forelse ($branches as $item)
                <a href="{{ route('turf-owner.filter.products', ['id' => $item->id, 'name' => $item->name]) }}"
                    class="px-3 py-1 m-2 border rounded-sm bg-primary hover:bg-green-700 text-white">{{ $item->name }}</a>
            @empty
